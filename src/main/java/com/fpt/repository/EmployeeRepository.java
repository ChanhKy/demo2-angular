...
public interface EmployeeRepository extends JpaRepository<Employee, Integer> {
	...
	@Query(value = " FROM Employee e "
			...
			+ "OR e.department.departmentName LIKE ?1 ) AND e.flag = False"
			)
	Page<Employee> searchByString(String str, Pageable page);
	

	Page<Employee> findAllByFlagFalse(Pageable pageable);
	
	Optional<Employee> findByEmployeeIdAndFlagFalse(int id);
	...
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query(value = "Update Employee SET flag = true WHERE employeeId = ?1")
	int deleteEmployeeByUpdate(int id);
	
	@Query(value = "FROM Employee e WHERE e.flag = 0 AND e.employeeId = ?1")
	Optional<Employee> getEmployeeByFlagAndId(int id);
	
	@Query(value = "FROM Employee e WHERE e.flag = false")
	List<Employee> danhSachNhanVienFlagFalse();
